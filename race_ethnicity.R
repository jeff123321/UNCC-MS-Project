data <- read.table("Weekly_counts_of_death_by_race.csv",sep=",",header=TRUE)
pop_perc <- read.table("race_percentages.csv",sep=",",header=TRUE)
labels <- c("Hispanic","Non-Hispanic American Indian or Alaska Native",
...
for(i in 1:6)
  pre_deaths[,i] <- data[
      ...
      data$Race.Ethnicity==labels[i],6]
